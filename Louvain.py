...
sys.stdout = open('E:\\脑电python3\\ceshi\\recode.log', mode='w', encoding='utf-8')
...
def fast_unfolding(vector_dict, edge_dict):
    ...
    for key in community_dict.keys():
        value_of_vector = community_dict[key]
        for i in range(len(value_of_vector)):
            vector_result[value_of_vector[i]] = int(vector_new_dict[str(key)])
    for key in vector_result.keys():
        vector_dict[key] = vector_result[key]
    print("Q_best : ", Q_best)
    print("vector_result.key : ", vector_dict.keys())
    print("vector_result.value : ", vector_dict.values())
    print("vector_result.value : ", list(vector_dict.values()))
    values = list(vector_dict.values())
    # print("vector_result.value-------type------", type(vector_dict.values()))
    return Q_best, values


# if __name__ == "__main__":
#     vector_dict, edge_dict = loadData("E:\\脑电python3\\ceshi\\node.txt")
